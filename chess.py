...
def common_draw(x,y,z,grap_true):
    ...
    if target:
        # print(target)
        servo_data = target[1]
        bus_servo_control.set_servos(joints_pub, 800, ((1, grap_true), (2, 500), (3, servo_data['servo3']), (4, servo_data['servo4']), (5, servo_data['servo5']), (6, servo_data['servo6'])))
        rospy.sleep(1)

def move_chess(qidian, zhongdian, chizi=0):
    # 0a
    #with open('armdict.txt','r') as rf:
        #armdic_text = rf.read()
        #armdict = eval(armdic_text)
    close = 230
    loose = 100
    loose_set = 160
    choose_item = chizi #判断吃子
    if choose_item == 0:
        x1,y1,z1 = qidian[0],qidian[1], qidian[2]
        x2,y2,z2 = zhongdian[0],zhongdian[1],zhongdian[2]
        dropz1 = zmode(z1)
        common_draw(x1,y1,z1,loose)
        rospy.sleep(1)
        common_draw(x1,y1,dropz1,loose)
        common_draw(x1,y1,dropz1,close)
        common_draw(x1,y1,z1,close)
        common_draw(x2,y2,z2,close)
        dropz2 = zmode(z2)
        common_draw(x2,y2,dropz2+0.011,close)
        common_draw(x2,y2,dropz2+0.011,loose_set)
        common_draw(x2,y2,dropz2+0.011,loose_set)
        common_draw(x2,y2,z2,loose)
        common_draw(-0.2,0.045,0.018,loose)
        #AK = ArmIK()
        #print(ik.getLinkLength())
        #target = AK.setPitchRanges((.20,.055,.018), -180, -180, 0)
        #if target:
        #    print(target)
        #    servo_data = target[1]
        #    bus_servo_control.set_servos(joints_pub, 1000, ((1, 700), (2, 500), (3, servo_data['servo3']), (4, servo_data['servo4']), (5, servo_data['servo5']), (6, servo_data['servo6'])))
        #   rospy.sleep(1)
        ...
        common_draw(x2,y2,z2,loose)
        rospy.sleep(1)
        common_draw(x2,y2,dropz2,loose)
        common_draw(x2,y2,dropz2,close)
        common_draw(x2,y2,z2,close)
        common_draw(-0.2,0.045,0.018,close)
        common_draw(-0.2,0.045,0.018,loose)
        common_draw(x1,y1,z1,loose)
        rospy.sleep(1)
        common_draw(x1,y1,dropz1,loose)
